...
{
	public static bool ColorEdit3Rgb(ReadOnlySpan<char> label, ref Formats.Types.Color.Rgb rgb)
	{
		...
		if (edited)
			rgb = new Formats.Types.Color.Rgb((byte)(vector.X * byte.MaxValue), (byte)(vector.Y * byte.MaxValue), (byte)(vector.Z * byte.MaxValue));

		return edited;
	}

	public static bool ColorEdit4Rgba(ReadOnlySpan<char> label, ref Formats.Types.Color.Rgba rgba)
	{
		...
		if (edited)
			rgba = new Formats.Types.Color.Rgba((byte)(vector.X * byte.MaxValue), (byte)(vector.Y * byte.MaxValue), (byte)(vector.Z * byte.MaxValue), (byte)(vector.W * byte.MaxValue));
